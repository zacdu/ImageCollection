...
///
/// This class uses the `UnsplashService` to fetch images based on a search query and to fetch individual images based on their URL. The fetched images are returned to the caller through completion handlers.
class ImageCollectionViewModel {
    ...
    ///
    /// This method uses the `UnsplashService` to fetch images based on the specified search query. Once the images are fetched, it calls the provided completion handler.
    ///
    /// - Parameters:
